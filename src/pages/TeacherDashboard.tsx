    ...
    socket.on('answer-received', (data) => {
      setResults(prev => {
        ...
        if (data.questionType === QuestionType.MULTIPLE_CHOICE) {
          ...
          const existingWordIndex = wordCloudData.findIndex((item: any) => 
            item.text.toLowerCase() === data.answer.toLowerCase()
          );
          ...
          if (existingWordIndex >= 0) {
            ...
            newWordCloudData = [...wordCloudData, { text: data.answer, value: 1 }];
          }
      ...
      {room && (
        <div className="mb-8">
          <div className="card mb-4">
            ...
            <p className="mb-2">Room ID: {room.id}</p>
          ...
          <div className="mb-8">
            ...
            {room.questions.length === 0 ? (
              <p className="text-muted">No questions created yet.</p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {room.questions.map((question) => (
                  <div key={question.id} className="card">
                    <h3 className="text-lg mb-2">{question.text}</h3>
                    <p className="text-sm text-muted mb-4">Type: {question.type}</p>
                    <button
                      className={`btn ${question.id === room.activeQuestionId ? 'btn-secondary' : 'btn-primary'}`}
                      onClick={() => handleActivateQuestion(question.id)}
                      disabled={question.id === room.activeQuestionId}
                    >
                      {question.id === room.activeQuestionId ? 'Active' : 'Activate'}
                    </button>
                  </div>
                ))}
              </div>
