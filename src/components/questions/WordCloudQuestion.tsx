  ...
  useEffect(() => {
    if (isTeacher && results.length > 0 && cloudContainerRef.current) {
      ...
      results.forEach((item) => {
        ...
        wordElement.style.fontSize = `${Math.max(
          12,
          Math.min(48, item.value * 5)
        )}px`
    ...
    <div className="card">
      ...
      {isTeacher ? (
        <div>
          ...
          >
            {results.length === 0 && (
              <p className="text-muted">{t('wordCloudQuestion.waiting')}...</p>
            )}
          </div>
          <div className="mt-4 text-sm text-muted">
            {results.length}{' '}
            {results.length === 1
              ? t('wordCloudQuestion.response')
              : t('wordCloudQuestion.responses')}{' '}
            {t('wordCloudQuestion.received')}
          </div>
        </div>
      ) : (
        <div>
          {!submitted ? (
            <>
              ...
              >
                {t('wordCloudQuestion.submitAnswer')}
              </button>
            </>
          ) : (
            <p className="text-green-600">
              {t('wordCloudQuestion.submitMessage')}
            </p>
          )}
        </div>
      )}
